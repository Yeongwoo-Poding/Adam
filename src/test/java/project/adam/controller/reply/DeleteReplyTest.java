...
public class DeleteReplyTest {
    ...
    void delete_reply() throws Exception {
        ...
        // when
        ResultActions actions = mvc.perform(delete("/replies/" + reply.getId()));
    ...
    @Test
    @WithMockUser(username = "email", password = CURRENT_MEMBER_ID)
    @DisplayName("대댓글 삭제 - DELETE - commentId가 문자인 경우")
    void delete_comment_id_is_string() throws Exception {
        ...
        // when
        ResultActions actions = mvc.perform(delete("/replies/" + "not_number"));
